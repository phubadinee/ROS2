...
class TwistNode(Node):
    def __init__(self):
        ...
        self.vel_pub = self.create_publisher(Twist, "cmd_vel", qos_profile=qos.qos_profile_sensor_data)
        self.vel_timer = self.create_timer(0.2, self.vel_timer_callback)
        self.vel_cmd = Twist()
        print("twist node is started")
    ...
    def random_vel(self):
        ...
        print(f"Linear x : {linear_x} , Angular_z : {angular_z}")
